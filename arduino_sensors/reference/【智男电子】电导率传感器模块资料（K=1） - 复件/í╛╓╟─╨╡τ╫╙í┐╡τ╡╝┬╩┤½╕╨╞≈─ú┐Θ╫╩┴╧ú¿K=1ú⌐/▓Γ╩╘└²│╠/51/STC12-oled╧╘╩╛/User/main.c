 ...
 *         D0~D7 -> P0口
 * EC传感器模块: VCC -> 5V; GND -> GND; AO -> P1.3;
 * 温度传感器DS18B20：VCC-> 5V;GND -> GND ;T -> P1.5
 * 功能描述：测量EC值、温度值液晶显示；
             串口接收测量所得的EC值（波特率9600）；
 *           可连接上位机显示EC值；
...
void EC_Value_Conversion()
{
	...
	EC_value=EC_value/compensationCoefficient;
	  ...
	  if((EC_value<=0)){EC_value=0;}
		...
		OLED_ShowChar(72,2,disbuff[4]);//显示ASCII字符	
}
...
void TEMP_Value_Conversion()
{
	  ...
	  disbuff[9]=(int)(temp_data)%10+'0';
		...
		OLED_ShowChar(24,4,disbuff[5]);//显示ASCII字符
		OLED_ShowChar(33,4,disbuff[6]);//显示ASCII字符
		OLED_ShowChar(42,4,disbuff[7]);//显示ASCII字符
		OLED_ShowChar(51,4,disbuff[8]);//显示ASCII字符	
	  OLED_ShowChar(60,4,disbuff[9]);//显示ASCII字符	
...
{ 
	...
	while(1)
	{
		...
		for(i=0;i<10;i++)   //发送数据至串口，波特率9600
				{
					R_S_Byte(disbuff[i]);
				}	
				R_S_Byte('\r');//发送回车换行符
